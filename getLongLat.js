...
"HONDROS COLLEGE OF NURSING - TOLEDO/MAUMEE",
...
async function run(){
  ...
  for(let i=0; i<list.length; i++){
    ...
    await page.type('input.gLFyf.gsfi', list[i] + ' coordinates')
    page.keyboard.press("Enter")

    let coordinate;
    try {
        await page.waitForSelector(".Z0LcW")
        coordinate = await page.evaluate(() => document.querySelector('.Z0LcW').innerText)
    } catch (err){
    ...
    coordinates.push({name:school_name, coordinate});
  }
    await browser.close();
    return coordinates;
}
...
const init = async () => {
	try {
		const gotCoordinate = await run();
		fs.writeFileSync('assets/coordinates.json', JSON.stringify(gotCoordinate));
		console.log('Done!');
	} catch (error) {
